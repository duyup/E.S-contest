...
{
    try {
        char detectname[BUFSIZE] = { 0, }; //라벨을 저장할 배열 선언
        FILE* fp; //fwrite함수에서 쓸 파일 포인터 선언
        ...
        fp = fopen("object.txt","w"); // TXT파일 문장을 초기화
        fclose(fp);
            ...
            for (j = 0; j < classes; ++j) {
                ...
                if (dets[i].prob[j] > thresh && show) {
                    if (class_id < 0) {
                        ...
                        if (strcmp(detectname, names[j])!= 0) // names[j]의 문자가 있다면
						{                                                   
                            fp = fopen("object.txt","w"); //TXT파일 열기
                            fwrite(names[j], strlen(names[j]), 1, fp); // 라벨이 저장된 names[j]의 문자를 TXT파일에 저장         
                            fclose(fp);
						}    
